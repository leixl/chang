...
public class LuceneContent {
	 ...
	 * @return
	 */
	public static Document createDocument(EMovie m) {
		Document doc = new Document();
		doc.add(new Field(ID, m.getId().toString(), Field.Store.YES,
				Field.Index.NOT_ANALYZED));
		doc.add(new Field(TITLE, m.getTitle(), Field.Store.NO,
				Field.Index.ANALYZED));
		if (!StringUtils.isBlank(m.getRemark())) {
			doc.add(new Field(CONTENT, m.getRemark(), Field.Store.NO,
					Field.Index.ANALYZED));
		}
		return doc;
	}

	public static Query createQuery(String queryString, Date startDate, Date endDate, Analyzer analyzer)
	...
	public static void delete(Date startDate, Date endDate, IndexWriter writer)
		...
		writer.deleteDocuments(createQuery(null,  
	...
	public static final String RELEASE_DATE = "createDate";
